...
```
db/
├── migrations/           # Migration files (YYYYMMDDHHMMSS_name.sql) -- source of truth
│   ├── 20260117000000_baseline.sql
│   └── ...
└── schema.sql            # Generated dump of the above; nothing executes it
```

`schema.sql` is regenerated by `scripts/schema-dump.sh` and diffed by the
`verify-schema-dump` CI job, so it cannot drift from the migrations unnoticed —
see [db/README.md](../db/README.md#the-generated-schema-dump-dbschemasql).
