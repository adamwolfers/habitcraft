...
router.get('/me', jwtAuthMiddleware, async (req, res) => {
  try {
    const result = await pool.query(`SELECT ${USER_COLUMNS} FROM users WHERE id = $1`, [
      req.userId,
    ]);
...
router.put('/me', jwtAuthMiddleware, updateProfileValidation, async (req, res) => {
  try {
    ...
    if (!name && !email) {
      return res
        .status(400)
        .json({ errors: [{ msg: 'At least one field (name or email) is required' }] });
    }
...
router.delete('/me', jwtAuthMiddleware, accountDeleteLimiter, async (req, res) => {
  ...
  try {
    ...
    const userResult = await client.query('SELECT email, password_hash FROM users WHERE id = $1', [
      req.userId,
    ]);
